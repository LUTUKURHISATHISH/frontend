...
const Overview = ({ department }) => {
  ...
  useEffect(() => {
    const getData = async () => {
      ...
      try {
        const response = await axios.get(`${backendUrl}api/get-dept-pso/${department}`);
        if (response.status === 200 && response.data) {
          setData(response.data.dept_pso_data || []);
        } else {
      ...
      {!loading && !error && data.length > 0 && (
        <div className="col-12 container mb-5">
          <b className="about__title wow animate__fadeInUp" data-wow-duration="0.5s" style={{ fontSize: "20px" }}>
            <span className="hedingname_orange">Program Specific Outcomes (PSOs)</span>
          </b>
          <div class="degree-list-container">
            <ul class="degree-list">
              {data?.map((item, index) => (
                <li key={index}>
                  <span className="fw-bold me-2">PSO {index + 1} :</span>
