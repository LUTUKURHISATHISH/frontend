...
const Overview = ({ department }) => {
  ...
  useEffect(() => {
    const getData = async () => {
      ...
      try {
        const response = await axios.get(`${backendUrl}api/get-dept-vision/${department}`);
        if (response.status === 200 && response.data) {
          setData(response.data.dept_vm_data || []);
        } else {
      ...
      {!loading && !error && data.length > 0 && (
        <div className="col-12 container mb-5">
          <b className="about__title wow animate__fadeInUp" data-wow-duration="0.5s" style={{ fontSize: "20px" }}>
            <span className="hedingname_orange">Vision</span>
          </b>
          <p className="contentsize">{data[0]?.about || 'No information available'}</p>
        </div>
      )}
       {!loading && !error && data.length === 0 && (
